...
def check_range(range, count):
    # Check the count of characters is within the bounds 
    if count <= max(range) and count >= min(range):
        return True
    else:
        return False

def count_characters(character, passwd):
    # Get a count of all characters in the password, then return the count of the passed character
    count = Counter(passwd)
    return count[character]

def check_pt1_validity(character, range, passwd):
    if check_range(range, count_characters(character, passwd)):
        return True
    else:
        return False

def check_pt2_validity(character, range, passwd):
    # Check if non-zero-indexed characters in password are in a given position and store as a bool
    check_lower = bool(passwd[range[0]-1] == character)
    check_upper = bool(passwd[range[-1]-1] == character)
    # Check that only one value matches and Return True, return False if not.
...
def sanitise_input(input):
    # Take the input string and return a tuple of the range of values (in a list), the control character, and the password
    data = input.split()
    password = data[-1]
    range = data[0]
    value_range = [int(i) for i in range.split("-")]
    char = data[1].strip(":")
    return value_range, char, password

for item in input:
    san = sanitise_input(item)
    if check_pt1_validity(san[1], san[0], san[2]):
        valid_pt1_passwds += 1
    
    if check_pt2_validity(san[1], san[0], san[2]):
        valid_pt2_passwds += 1

print(valid_pt1_passwds)
print(valid_pt2_passwds)